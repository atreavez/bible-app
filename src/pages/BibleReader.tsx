...
import { ChevronLeft, ChevronRight, Volume2, VolumeX, Search, Bookmark, BookmarkCheck } from "lucide-react";
...
export default function BibleReader() {
  ...
  const filteredBooks = bibleBooks.filter(
    (b) => b.testament === activeTestament && b.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            <div className="ornate-border rounded-2xl bg-card/80 p-4 mb-6 flex items-center justify-between">
              ...
              <div className="text-center flex-1">
                ...
                <div className="mt-3 flex items-center justify-center gap-1.5 flex-wrap">
                  {Array.from({ length: Math.min(selectedBook.chapters, 50) }, (_, i) => i + 1).map((ch) => (
                    <button
                      key={ch}
            ...
            <div className="mb-6 flex justify-end">
              <button
                ...
                className={`ornate-border-hover px-5 py-2.5 rounded-xl flex items-center gap-2 font-body text-sm transition-all duration-300 ${
                  isReading ? "bg-olive text-primary-foreground" : "bg-card text-foreground hover:text-olive"
                }`}
              >
                ...
                {loading ? (
                  ...
                  >
                    <h3 className="font-display text-lg text-muted-foreground mb-6 text-center">
                      {selectedBook.name} Chapter {selectedChapter} · {selectedTranslation.abbreviation}
                    </h3>
