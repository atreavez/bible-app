...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are a lyrics verification assistant. Return the EXACT full official lyrics for the song "${title}"${artist ? ` by ${artist}` : ""}.
...
- Preserve the original line breaks of the song
  - If you are not highly certain the lyrics are exact, return exactly: __LYRICS_NOT_FOUND__
...
- Do NOT say "here are the lyrics" or anything similar`;
    ...
    if (!lyrics || lyrics === "__LYRICS_NOT_FOUND__") {
      return new Response(
        JSON.stringify({
          success: false,
          error: "Verified lyrics unavailable",
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
